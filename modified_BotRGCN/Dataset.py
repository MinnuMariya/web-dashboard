...
class Twibot20(Dataset):
    ...
    def __init__(self, root: str = './Data/', device: str = 'cpu', process: bool = True, save: bool = True):
        """
        ...
        if process:
            ...
            df_dev = pd.read_json(f'{root}Twibot-20/dev.json')
            df_dev = df_dev[df_dev['domain'].apply(lambda x: x == ['Politics'])]
    ...
    def tweets_embedding(self) -> torch.Tensor:
        ...
        if not os.path.exists(path):
            ...
            for each_person_tweets in tqdm(tweets):
                total_each_person_tweets = torch.zeros(768)  # Assuming RoBERTa-base output size
    ...
    def num_prop_preprocess(self) -> torch.Tensor:
        ...
        if not os.path.exists(num_properties_path):
            ...
            followers_count = [profile.get('followers_count', 0) for profile in self.df_data['profile']]
            friends_count = [profile.get('friends_count', 0) for profile in self.df_data['profile']]
            screen_name_length = [len(profile.get('screen_name', '')) for profile in self.df_data['profile']]
            favourites_count = [profile.get('favourites_count', 0) for profile in self.df_data['profile']]
            ...
            active_days = [(reference_date - dt.strptime(profile.get('created_at', reference_date.strftime('%a %b %d %X %z %Y')), '%a %b %d %X %z %Y')).days if profile else 0 for profile in self.df_data['profile']]
